...
double T1, T2;
//as we only want to wait for the Irecv, we change nreqs from REPS*2 to REPS
...
if (rank < 2) {
  ...
  if (rank == 1) {
    src = 0;
    //offset set to 0 due to overflowing reqs as offest increments
    offset = 0;
    }
  dest = src;
...
/* Do the non-blocking send and receive operations */
  for (i=0; i<REPS; i++) {
    ...
    //Add an MPI wait for each Irecv, as we only need to wait for those so as not to block the Isend.
    //Note the index choice as after offset has incremented.
...
/* Task 3 does the non-blocking receive operation */
  if (rank == 3) {
    ...
    for (i=0; i<REPS; i++) {
      ...
      offset += 1; //changed to increment offset by 2 to duplicate used entries
      if ((i+1)%DISP == 0)
...
//Commented out the Waitall for future reference, as it is unnecessary to wait for the Isend for rank < 2.
