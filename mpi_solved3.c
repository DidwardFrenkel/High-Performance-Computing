...
//No MPI_Init. Added accordingly, and at a location where it accounts for functions that call 
//elements that are defined later in the function.
MPI_Init(&argc,&argv);
...
/* No MPI Finalize. Added accordingly at end of main, where no more MPI functions
 * will be called after each run on each process.
 */
MPI_Finalize();
