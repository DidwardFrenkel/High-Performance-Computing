...
if (rank == 1) {
  for (i=0; i<10; i++) {
    /* Changed MPI_FLOAT to MPI_INT
     * Added an int gamma to receive the Isend and set beta by converting gamma to a float.
     */
    int gamma;
    MPI_Irecv(&gamma, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &reqs[i]);
    beta = (float)gamma;
    MPI_Wait(&reqs[i], &stats[i]);
